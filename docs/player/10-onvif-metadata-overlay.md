...
**Version:** 1.1.14 · **Author:** Youngho Kim · **Milestone:** M-4

**History**

| Date | Change |
| --- | --- |
| 2026-09-09 | REQ-PLY-115 revised: the "ONVIF Event" toggle row is now always present in the context menu instead of staying `display: none` until the first `VideoAnalytics` frame — reported live as the control having "disappeared" on a stream/profile that never sends ONVIF analytics metadata, indistinguishable from a bug. `applyOnvifOverlayMenuState()` now always sets `display: flex` and instead toggles a new `onvifOverlayStateElement` (`.onvif-overlay-state-dot`, same dot+`title` convention as the Audio group's `audioToggleStateElement`) between "Waiting for ONVIF metadata" / "Receiving ONVIF metadata". The switch (`onvifOverlaySwitch`) stays interactive in both states — turning it on before any metadata has arrived is harmless, `renderOnvifOverlay()` already no-ops with nothing cached. See `docs/SRS.md`'s REQ-PLY-115, `docs/TC.md`'s TC-PLY-116/117, and `MEMORY.md`. |
