...
**Version:** 1.2.10 · **Author:** Youngho Kim
...
| 2026-09-08 | Fixed a ninth real bug, reported directly by the user: Live mode playback would repeatedly cycle Pause/Play/Pause after the browser tab/window was minimized and restored, not just stall once. `onVisibilityChange()`'s Live-mode catch-up (added for the eighth-bug-adjacent fix, reuses `videoUpdating()`) jumped `currentTime` to `endTime - defaultDelay` — a 0.3-0.7s margin tuned for nudging *steady-state* jitter, not for resuming after a real background gap, where the decoder may not have anything actually playable ready that close to the buffered edge yet. Landing there re-fired `'waiting'`, which called `videoUpdating()` again, computed the identical too-small jump again, and repeated indefinitely. Fixed by arming a new `visibilityResumeUntil` deadline (`performance.now() + VISIBILITY_RESUME_COOLDOWN_MS`, 5s) in `onVisibilityChange()`'s Live branch — while active, `videoUpdating()`'s Live-branch catch-up jump (from *any* trigger: `'waiting'`, `'updateend'`, `'durationchange'`, not just the visibility-restore call itself) uses `defaultDelay * VISIBILITY_RESUME_DELAY_MULTIPLIER` (5x) instead of the bare steady-state margin, giving the decoder real runway across however many retries it actually takes; reverts to normal steady-state behavior once the cooldown expires. Not yet verified against a real device. |
| 2026-09-08 | Fixed a tenth real bug, found from the ninth fix's own chrome://media-internals trace: after `DEMUXER_UNDERFLOW`, one seek would cascade into a "domino" of dozens of `Seek`/`kPipelineStateChange` pairs within the same handful of milliseconds, `currentTime` creeping forward by roughly one segment's duration each time, before settling into plain Play/Pause cycling with no more seeking at all. Root cause: `videoUpdating()`'s Live-branch catch-up jump runs on *every* `sourceBuffer` `'updateend'` (one per MSE append) with no rate limit — when a backlog of many segments drains in a burst (a queue built up during a stall, or right after the ninth fix's own visibility-resume catch-up), each `updateend` sees `endTime` a little further along than the previous one and immediately re-jumps `currentTime` to chase it, every single time, often only milliseconds apart — playback never gets a chance to actually render a frame before being yanked forward again. Fixed with a new `lastLiveCatchUpJumpAt` timestamp: the jump (and its `videoPlay()` call) now only actually executes if at least `MIN_LIVE_CATCHUP_JUMP_INTERVAL_MS` (500ms) has passed since the last one; the `bufferedFrameCount` NVR tuning below it is unaffected. The later plain Play/Pause-with-no-seeking phase this bug's own report also described is a separate, likely non-client-side symptom — see `MEMORY.md`'s matching entry for why. Not yet verified against a real device. |
| 2026-09-08 | Fixed two real, independent memory-leak bugs, reported directly by the user as Live-session memory exceeding 1GB despite `checkBufferSize()`'s existing `getMaxInstantPlaybackTime()`-based trimming (which the user correctly suspected might not actually be working as intended). **Bug one**: `checkBufferSize()`'s trim-trigger check measured `endTime - startTime` from `sourceBuffer.buffered`'s *last range only* (`buffered.start/end(buffered.length - 1)`), not the true total buffered span. `SourceBuffer.buffered` can legitimately fragment into more than one range (a PTS discontinuity between segments, or the currentTime-jump-driven fragmentation the ninth/tenth fixes above document) — once fragmented, the last range's own width can stay small indefinitely while everything in the *earlier* ranges keeps growing completely untrimmed, since this gate never saw it. `sourceBuffer.remove(0, removeEnd)` itself was always correct (MSE removes across every intersecting range regardless of fragmentation) — only the gating measurement was wrong. Now measures `buffered.start(0)` to the last range's own `end()`, the true earliest-to-latest span. **Bug two**: `makeOnCueChange()`'s safety-net cue-count cap (`MAX_CUE_COUNT` = 100) — meant to catch `VTTCue`s orphaned by a currentTime seek skipping over their `[startTime, endTime)` range entirely (so `onCueExit()`, the normal removal path, never fires for them) — had an inverted condition, `cues.length < i` inside a loop that only runs after already confirming `cues.length > MAX_CUE_COUNT`, which can never be true for any `i` in `[0, MAX_CUE_COUNT)`; `removeCue()` there was dead code. Every seek-orphaned cue accumulated for the rest of the session with nothing ever trimming them — and this session's own tenth-bug fix (frequent Live-branch catch-up jumps) is itself a likely source of many such orphaned cues. Fixed by removing `cues[0]` (the oldest, since cues are always appended in chronological order) exactly `cues.length - MAX_CUE_COUNT` times, the same pattern `removeAllCues()` already uses. See `MEMORY.md` for the full narrative. Not yet verified against a real device. |
| 2026-09-08 | Fixed a third memory-accumulation bug in the same investigation, reported by the user as memory still exceeding 1GB after the two fixes above: `boxStartTime` was appended to directly (`this.boxStartTime.push(...)`, once per segment) for the entire life of a Live/Playback session, with nothing ever trimming it during active playback — only `close()` ever cleared it. Its only reader, `changeCurrentTime()`, never looks back further than `boxTimeIndex` entries from the end (at most 2, i.e. `length - 1 - 2`), so every older entry was pure dead weight, growing without bound for as long as a session ran. New `pushBoxStartTime()` replaces the two direct `.push()` call sites and caps the array at `MAX_BOX_START_TIME_ENTRIES` (10, well past the 2-back the reader ever needs) via `.shift()`. Being a small-number array (not large `Uint8Array` buffers), this alone was very unlikely to be the dominant contributor to a multi-hundred-MB/1GB figure — flagged to the user as a real fix worth having regardless, alongside asking how memory is actually being measured (a single tab's own footprint vs. the whole browser process tree) and whether it's still climbing without bound or plateauing, since neither of these three fixes so far accounts for gigabyte-scale growth by itself. Not yet verified against a real device. |
| 2026-09-08 | Fixed the actual dominant contributor to the same 1GB leak, found once the user confirmed (via both Chrome's and Windows' Task Managers) that memory climbs continuously with no plateau, and specifically called out that it gets noticeably worse right after the system enters and recovers from a power-saving state (Windows "Efficiency Mode" / Chrome Energy Saver) — the same underlying mechanism `onVisibilityChange()`'s existing fixes above already document (RTP delivery/segment creation keep running while some other part of the pipeline is suspended), just affecting the *append* side instead of `currentTime`. `segmentArray` (already-muxed `moof`+`mdat` `Uint8Array` buffers awaiting `appendSegmentToSourceBuffer()`) had no size limit at all — in steady state one entry drains per `'updateend'` about as fast as they're produced, so this rarely mattered, but if the browser suspends the tab's MSE append pipeline for an extended period while segment creation keeps running regardless, nothing ever bounded how large this backlog could grow, and unlike the three fixes above (small fixed-size arrays of numbers/cue objects), this one holds real encoded frame data — the only one of the four capable of reaching gigabyte scale on its own. Fixed with a new `pushSegment()`, used at every non-init push site instead of `.push()` directly: once `segmentArray` reaches `MAX_SEGMENT_QUEUE_LENGTH` (500), a new segment is dropped rather than queued — the same backpressure-by-dropping trade-off `MJPEG_ENCODER_MAX_QUEUE_SIZE` already makes elsewhere in this class, strictly better than unbounded growth. Deliberately drops the *incoming* segment rather than `shift()`-ing an old one from the front to make room, since `createInitSegment()`'s own `unshift()` can leave a not-yet-appended init segment sitting at index 0 that a front-eviction policy could otherwise discard. Not yet verified against a real device. |
| 2026-09-08 | Found and fixed the real reason `checkBufferSize()`'s 30s trim wasn't actually capping growth during exactly the Efficiency-Mode-recovery scenario the fix above targeted — reported by the user as memory still jumping ~200MB (600MB -> 800MB) on recovery even with `pushSegment()`'s cap in place. Root cause: `sourceBufferEventListener`'s `'updateend'` case called `appendSegmentToSourceBuffer()` *before* `videoUpdating()` (which is what actually invokes `checkBufferSize()`). `appendBuffer()` sets `sourceBuffer.updating` back to `true` synchronously the instant it's called (MSE spec) whenever another segment is queued — so on every single `'updateend'` during a backlog-draining burst (the common case right after a suspended append pipeline resumes), the next append had *already* been kicked off a few lines earlier in the very same handler by the time `checkBufferSize()`'s `!sourceBuffer.updating` guard ran, so it always saw `true` and skipped the trim — not occasionally, for the *entire* burst. Trimming only ever resumed once the backlog fully drained and appends returned to their normal one-per-real-time-interval cadence, by which point the buffer had already grown by however much the burst itself added — exactly the "600MB -> 800MB right on recovery" shape reported. Fixed by swapping the order: `videoUpdating()` now runs first, at the exact instant `'updateend'` guarantees `updating` is `false` (the just-completed operation, whichever it was, has finished and nothing new has been kicked off yet in this handler) — so the trim check gets a real chance to run on every completed operation, burst or not, rather than being starved by the very backlog it exists to bound. `remove()` and `appendBuffer()` interleave correctly afterward regardless of which one `checkBufferSize()` decides to start this cycle, since a `remove()` completion is itself another `'updateend'` that re-enters this same handler. Not yet verified against a real device. |
| 2026-09-08 | Added a temporary `logMemoryDiagnostics()` (called once per second from the existing `getCurrentVideoFrame()`/`statisticsTimer` tick, gated through `debugLog` like every other trace in this class) after the user reported the same 1GB-class leak still reproducing in a plain single-camera Live session with no Efficiency Mode/Instant Playback/multi-camera involvement — reaching ~800MB within under 2 minutes even with all four fixes above applied and reloaded, meaning none of those four mechanisms explain it. Logs `segmentArray`/`videoSamples`/`audioSamples`/`boxStartTime` lengths, the `TextTrack` cue count, `sourceBuffer.buffered`'s actual total span (should stay near `getMaxInstantPlaybackTime()`, 30s by default, if trimming is keeping up), and — where available — `performance.memory.usedJSHeapSize` (Chrome-only/non-standard), to directly answer whether growth is JS-heap-side (one of this class's own arrays still unbounded somewhere not yet found) or browser/GPU-internal decode-pipeline memory this class has no visibility into at all. To be stripped once the actual leak source is confirmed. |
| 2026-09-08 | Fixed a real regression the `videoUpdating()`-before-`appendSegmentToSourceBuffer()` reorder (two entries above) itself introduced: the debug `[trace]` log the diagnostics entry above enabled showed `checkBufferSize trimming: buffered=30.3Xs, remove(0, ...)` firing on essentially *every single* `'updateend'` — the `updateend` trace counter climbed by tens of thousands while `durationchangeCount` (real new data extending the timeline) never moved at all, meaning real playback progress had completely stopped. Root cause: `remove(0, removeEnd)` always trims down to exactly `currentTime - getMaxInstantPlaybackTime()`, so the buffered span settles *just barely above* the bare threshold (observed live: a constant ~30.3-30.4s, never below 30s) — with no margin, `checkBufferSize()`'s outer `if (span > getMaxInstantPlaybackTime())` check stayed true on the very next call too, even though the previous `remove()` had nothing meaningful left to remove. Combined with the reorder fix (`checkBufferSize()` now runs *before* `appendSegmentToSourceBuffer()` every `'updateend'`), this created a genuine livelock: a near-no-op `remove()`'s own completion fires another `'updateend'`, which re-triggers another near-no-op `remove()`, indefinitely, as fast as the browser can cycle — starving real appends completely, not just delaying them, and burning CPU/memory the whole time. Fixed two ways: (1) `CHECK_BUFFER_SIZE_HYSTERESIS_SECONDS` (5) — the outer condition now requires the span to exceed `getMaxInstantPlaybackTime() + 5`, so a just-trimmed buffer stays quiet until genuinely new data re-accumulates, rather than re-triggering on its own trimmed-but-still-"over-threshold" state; (2) `lastCheckBufferSizeTrimAt`/`MIN_CHECK_BUFFER_SIZE_TRIM_INTERVAL_MS` (1000ms) — a hard floor on how often an actual `remove()` can fire at all, independent of the hysteresis math, as defense-in-depth against a similar bug recurring in the future (degrades to "trims slightly less often than ideal" instead of "livelocks the player"). Reported live by the user pasting the raw `[trace]` console output twice, which is what surfaced this. Not yet verified against a real device that this actually restores playback progress (only that the log pattern's own mechanism is now broken). |
...
- **Method Analysis — fMP4 muxing / MSE pipeline.**
  ...
  - `sourceBufferEventListener('updateend')` (`:350-364`) — drains any pending `clearBufferFlag`
    ...
    since MSE only allows one `appendBuffer` in flight at a time. Also calls `videoUpdating()`
    unconditionally on every `updateend` (both Live and Playback, see the next bullet).
  - `videoUpdating()` (`:1497-1590`) — the live-edge/latency controller: reads
    `sourceBuffer.buffered`'s start/end, computes `latency = endTime - currentTime`, and nudges
    `videoElement.currentTime` forward (skipping ahead) when latency exceeds `this.delay`, or backs
    off `bufferedFrameCount` when latency is comfortably low — effectively an adaptive jitter buffer
    implemented via `<video>` seeking rather than an actual audio/video decoder queue. Also calls
    `checkBufferSize()` (`:1475-1495`) to `sourceBuffer.remove()` old buffered ranges once they
    exceed `getMaxInstantPlaybackTime()`, capping MSE buffer growth. Triggered from both
    `onDurationChange()` (the `<video>` element's native `durationchange` event) and every
    `sourceBuffer` `updateend` — originally `durationchange`-only, but `durationchange` was found
    live to stop firing entirely after the first handful of appends against a continuously-growing
    fragmented-MP4 `SourceBuffer` (an MSE implementation detail, not something this class controls).
    A first fix scoped the `updateend` trigger to Playback only (`playbackFlag`), since only
    Playback had been reported broken at the time; extended to Live too once the same
    `durationchange` stall was suspected as the root cause of unbounded Live-mode `SourceBuffer`
    growth (`checkBufferSize()`'s trim never running once `durationchange` goes quiet) — see
    `MEMORY.md`. Temporary `console.log('[VideoTagPlayer][trace] ...')` tracing was added at
    `onDurationChange()`, the `updateend` handler, and `checkBufferSize()`'s actual `remove()` call
    sites to confirm the stall and the fix live against a real device; to be stripped once
    confirmed. Also triggered from `onVisibilityChange()`'s Live branch on tab/window refocus (see
    that method's own entry below) and `onWaiting()`'s Live `else` branch on a native `'waiting'`
    event.
  - **`onVisibilityChange()`'s Live-branch catch-up, and the Pause/Play/Pause loop it could cause,
    added 2026-09-08.** Reported directly by the user: minimizing then restoring the browser during
    Live playback caused indefinite Pause/Play/Pause cycling, not a one-time stall. This branch
    (`this.videoUpdating()`, reused rather than duplicated — see the eighth-bug History entry for
    the analogous Playback-mode `changeCurrentTime()` fix) inherits `videoUpdating()`'s Live-branch
    jump target, `endTime - defaultDelay` — a 0.3-0.7s margin sized for nudging small steady-state
    jitter, not for resuming after a real background gap where the decoder may have nothing
    actually playable that close to the buffered edge yet. Landing there re-fired `'waiting'`,
    which called `videoUpdating()` again, recomputed the identical too-small jump, and repeated.
    Fixed with a new `visibilityResumeUntil` field: `onVisibilityChange()`'s Live branch arms it to
    `performance.now() + VISIBILITY_RESUME_COOLDOWN_MS` (5s) before calling `videoUpdating()`; while
    armed, `videoUpdating()`'s Live-branch jump target uses `defaultDelay *
    VISIBILITY_RESUME_DELAY_MULTIPLIER` (5x) instead of the bare `defaultDelay`, for *every* call
    during that window — not just the one `onVisibilityChange()` itself triggers — since a single
    one-shot larger margin on only the first call wouldn't help if that first jump still isn't
    enough and a later `'waiting'`/`'updateend'` retry falls back to the small margin again. Not yet
    verified against a real device.
  - **Live-branch catch-up jump rate limit, added 2026-09-08.** Direct follow-up, same
    investigation: the user's own chrome://media-internals trace after the ninth-bug fix above
    showed that once `DEMUXER_UNDERFLOW` recovery kicked in, one seek would cascade into a "domino"
    of dozens of `Seek`/`kPipelineStateChange` pairs within the same handful of milliseconds
    (`currentTime` creeping forward by roughly one segment's duration each time), then settle into
    plain Play/Pause cycling with no more seeking at all. Root cause: `videoUpdating()`'s Live-branch
    jump has no rate limit and runs on every single `'updateend'` (one per MSE append) — while a
    backlog of many segments drains in a burst (a queue built up during any stall, including right
    after the ninth fix's own visibility-resume catch-up), each `updateend` sees `endTime` a little
    further along than the previous call and immediately re-jumps to chase it, often only
    milliseconds apart, never letting playback actually render a frame before being yanked forward
    again. Fixed with a new `lastLiveCatchUpJumpAt` timestamp (`performance.now()` of the last actual
    jump, `0` = never) — the jump (and its `videoPlay()` call) only executes if at least
    `MIN_LIVE_CATCHUP_JUMP_INTERVAL_MS` (500ms) has passed since the last one; the `bufferedFrameCount`
    NVR-tuning statement below it still runs on every call, unaffected. The later plain
    Play/Pause-with-no-seeking phase the same report described is a separate, likely
    not-client-side-fixable symptom (see `MEMORY.md`'s matching entry) — probably the browser's own
    native buffering pause/resume reacting to genuinely insufficient real-time data arrival, which no
    amount of `currentTime` correction logic can manufacture data for. Not yet verified against a
    real device.
  - **`checkBufferSize()`'s last-range-only trim gate, added 2026-09-08.** Direct follow-up, same
    investigation: reported directly by the user as Live-session memory exceeding 1GB despite this
    method's `getMaxInstantPlaybackTime()`-based trim existing. Its trigger check measured
    `endTime - startTime` from `sourceBuffer.buffered`'s *last range only*
    (`buffered.start/end(buffered.length - 1)`) — but `buffered` can fragment into more than one
    range (a PTS discontinuity between segments, or the currentTime-jump-driven fragmentation the
    two bullets above document), and once fragmented the last range's own width can stay small
    indefinitely while everything buffered in *earlier* ranges keeps growing completely untrimmed,
    since this gate never measured them at all. The actual `sourceBuffer.remove(0, removeEnd)` call
    was always correct — MSE removes across every range intersecting `[0, removeEnd)` regardless of
    fragmentation — only the gating measurement was wrong. Now uses `buffered.start(0)` (the true
    earliest buffered position across every range) instead of the last range's own `start()`. Not
    yet verified against a real device.
  - **`makeOnCueChange()`'s inverted cue-count-cap condition, added 2026-09-08.** Second,
    independent bug found in the same investigation. This handler's whole purpose is a safety-net
    cap (`MAX_CUE_COUNT` = 100) for `VTTCue`s a currentTime seek skips clean over — `onCueExit()` is
    the normal removal path, but a jump whose target lands past a cue's entire
    `[startTime, endTime)` range means the browser's "time marches on" algorithm never fires
    `onexit` for it, orphaning it in the `TextTrack` permanently. The cap's own condition,
    `cues.length < i` inside a loop that only runs once `cues.length > MAX_CUE_COUNT` is already
    confirmed, can never be true for any `i` in `[0, MAX_CUE_COUNT)` — `removeCue()` there was dead
    code, so every orphaned cue accumulated for the rest of the session, uncapped. The two
    catch-up-jump bugs immediately above are themselves a likely source of many such orphaned cues.
    Fixed by removing `cues[0]` (cues are always appended in chronological order, so index 0 is
    always the oldest remaining one — same assumption `checkTimestampCueAtCurrentTime()` already
    documents) exactly `cues.length - MAX_CUE_COUNT` times, the same removal pattern
    `removeAllCues()` already uses elsewhere in this class. Not yet verified against a real device.
  ...
  - `setAudioInfo(audioinfo)` (`:2262-2347`) — the audio-codec-switch handler: refuses to switch
    ...
    **Follow-up, real bug found live (2026-09-07)**: the fix above only pre-seeded `opusActive`
    (which picks the `SourceBuffer`'s MIME `codecs` string) — it left `this.audioInfo` (which
    controls what `createInitSegment()` actually *writes* into the init segment's audio `stsd`
    entry, class-field-defaulted to `codecType: 'AAC'`) unseeded. If the first video I-frame won
    the arrival race (the common case, not a rare one), the init segment ended up declaring
    `opus` in the MIME type while its `stsd` box still held an AAC `esds` — Chrome's demuxer
    rejects this outright (`CHUNK_DEMUXER_ERROR_APPEND_FAILED: "audio object type 0x40 does not
    match what is specified in the mimetype"`), closing the `MediaSource` with the `<video>`
    element showing nothing, silently (see the masking bug at `videoElementEventListener` below).
  ...
  - `videoElementEventListener('error')` (`:495-`) — **real bug, found live (2026-09-07)**: every
    ...
    from the player simply going idle. This is what let the `opusActive`/`audioInfo` bug above (and
    the `close()`/reconnect investigation two History entries above it) go undiagnosed for so long
    — nothing ever surfaced the actual `MediaError`. Now reports `videoElement.error` (code +
