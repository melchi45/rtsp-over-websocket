...
| 2026-09-04 | Added `debug`-gated `console.log` tracing (`util/debugLog.ts`, `debug["mediaSession"]` in the new element `debug` attribute — see `01-elements-interface-exceptions.md`, `08-util.md`). `Session` (the shared base every `*Session` class extends, directly or via `RtpSession`) gained one generic `setDebugConfig(config, componentName)` + `protected debugLog` — a single change that instruments all 13 subclasses (`H264Session`/`H265Session`/`VP8Session`/`VP9Session`/`AV1Session`/`MjpegSession`/`G711Session`/`G726Session`/`OPUSSession`/`AACSession`/`AudioTalkSession`/`MetaSession`/`RTCPSession`, the last four documented in `04-mediaSession-audio-text.md`) without editing any of their files. `RtpClient.sendSdpInfo()` calls it right after each `new XSession()` + `.init(...)`, passing the exact literal class name it just built (11 call sites) — `componentName` is always a hardcoded string literal, never derived via `constructor.name`, since `build:player`'s minified output isn't guaranteed to preserve real class names. `MediaRouter`/`RtpClient`/`MetaDataParser` each also gained their own `debug`/`set debug()`; `MediaRouter.selectVideoPlayer()`/`sendCommandData()` and `RtpClient.sendSdpInfo()` got new trace points. `MediaRouter` additionally forwards `debug` to every video/audio/talk/backup/metadata-parser instance it hands out from `factories.createXxx()` (7 call sites) — see `05-video-tag-player.md`/`11-canvas-tag-player.md`/`06-listen-audio.md`/`07-talk-backup-worker.md` for what each of those does with it. |
| 2026-09-04 | Follow-up, requested directly by the user: the `debug` feature above wired up per-class *enable* gates but left the actual `depacketize()` hot path silent — no trace, `debug` or otherwise, existed at the point RTP packets actually arrive. Added: `RtpClient.sendRtpData()` (the per-packet dispatcher, `:sendRtpData`) gets one `debug`-level trace per packet (`interleavedId`/byte count/whether a session was found); `MediaRouter.handleVideoData()`/`handleAudioData()` each get one `debug`-level trace per already-depacketized frame handed up from a session. Every video session's own `depacketize()` (`H264Session`/`H265Session`/`VP8Session`/`VP9Session`/`AV1Session`) gets a `debug`-level trace per raw RTP packet plus an `info`-level trace when `flags.markerBit` completes a frame (frame type, assembled byte size, packet-sequence number) — `debug` therefore shows every wire packet, `info` (the default threshold) shows only the coarser "one line per frame" milestone. `MjpegSession` (worker-offloaded, no synchronous frame assembly in this class — see `11-canvas-tag-player.md`'s `decoderWorker` note) gets `debug` at `depacketize()`'s packet-queueing point and `info` at `handleWorkerMessage()`, the point a decoded frame actually comes back. `RTCPSession.depacketize()`/`parse()` get `debug` at packet entry and SDES, `info` at the sender-report (`eventRtcpCallback`) and BYE branches — the two RTCPSession/MjpegSession doc comments claiming "legacy debug logger calls not reproduced" are updated to note they're reintroduced now, gated, rather than the claim staying stale. See `04-mediaSession-audio-text.md` for the audio/text sessions' matching change, and `MEMORY.md`. |
| 2026-09-04 | Added `mediaSession`-only group aliases to `debug["mediaSession"]`'s string array — `"videoSession"`/`"audioSession"`/`"textSession"`/`"rtpSession"`/`"rtcpSession"`, mirroring the real `mediaSession/videoSession`, `audioSession`, `textSession`, `RtpSession.ts`, `RTCPSession.ts` split — instead of requiring every individual class name to be listed by hand (e.g. `debug["mediaSession"] = ["videoSession"]` now covers all 6 video codecs at once). Requested directly by the user. Implemented as a lookup table (`MEDIA_SESSION_GROUPS`, `util/debugLog.ts`) `isDebugEnabled()` expands when checking array membership — no schema change (`DebugTarget` stays `boolean \| string[]`, `validateDebugConfig` needed no edits since an alias is just another valid string), and no other subsystem has this concept. Verified live against a real camera: `debug["mediaSession"] = ["videoSession", "RtpClient", "MediaRouter"]` correctly gated `H264Session`'s new depacketize tracing (1308 `debug`-level packet lines + 61 `info`-level frame-complete lines for a real session) without naming `H264Session` directly. See `08-util.md`. |
| 2026-09-04 | Live-refresh: reported directly by the user, a `debug` config change made *during* an already-running stream had no visible effect — `MediaRouter.set debug()` and `RtpClient.set debug()` used to only apply to instances constructed *after* the setter ran. Both now also re-apply to whatever they're already holding: `MediaRouter` re-pushes to `_videoPlayer` (recomputing `'VideoTagPlayer'`/`'CanvasTagPlayer'` from its own `tagMode`, same as the original wiring), `audioPlayer`, `audioTalker`, `backupProvider`, `metaDataParser` if any already exist; `RtpClient` iterates every session already in `sessionArray` plus `audioTalkSession`, calling `session.setDebugConfig(config, session.debugComponentName)` — a new public `debugComponentName` field on `Session.ts` (set inside `setDebugConfig()` itself) that remembers the literal name a session was last configured with, so `RtpClient` doesn't need its own parallel bookkeeping of which name belongs to which already-constructed session. See `01-elements-interface-exceptions.md`'s matching History entry (the full chain starts at `RTSPOverWebSocket.ts`) and `MEMORY.md` for the complete per-class breakdown, including the two further hops handled in `05-video-tag-player.md`/`11-canvas-tag-player.md`/`06-listen-audio.md`. |
| 2026-09-08 | Memory-leak audit (`transport`/`mediaSession` first, per direct user request): fixed `MediaRouter.terminate()` never clearing the `minimapUpdateTimer` `setInterval` or closing an in-progress `backupProvider`, and `Session.removeEventListener()` silently no-op-ing for `'rtcp'`/`'waiting'`. See this file's `terminate()`/`addEventListener`/`removeEventListener` Method Analysis entries above, `02-network.md`'s matching `clearTransport()` entry (same audit, the `Transport` self-reconnect leak), and `MEMORY.md`. |
| 2026-09-08 | `AudioPlayerLike` gained an optional `audioEncoderMode?: string` field, and `createAudioPlayer()`/`set audioEncoderMode()` now push the `audioencodermode` attribute's value into it — extending that attribute from encode-only (`WebCodecsAudioEncoder`, talk-back) to also selecting the AAC **decode** tier in canvas-tag mode. Requested directly by the user, whose motivation was `vendor/ffmpegAAC.decoder.js`'s fixed 160MiB asm.js heap. See `createAudioPlayer`'s own bullet below, and `06-listen-audio.md`'s new `AACWebCodecsAudioDecoder` section for the decoder this selects. |
...
- `addEventListener(event, cb, extraInfo?)` / `removeEventListener(event)` — a fixed-vocabulary pub/sub (`'text' | 'video' | 'metaImage' | 'audio' | 'rtcp' | 'statistics' | 'waiting'`) that simply assigns to the matching `event*Callback` field; `'waiting'` additionally stashes `extraInfo.waitingTimeout`. This is how `RtpClient` wires each session's output into `MediaRouter`. **Fixed 2026-09-08**: `removeEventListener`'s switch was missing the `'rtcp'`/`'waiting'` cases entirely (only `'text'`/`'video'`/`'metaImage'`/`'audio'`/`'statistics'` cleared their callback), so `eventRtcpCallback`/`eventWaitingCallback` (and `eventWaitingTimeout`) could never be individually unsubscribed — only ever overwritten by a later `addEventListener` call or dropped along with the whole `Session` object once `RtpClient.close()`/`sessionArray = []` released it. Low real-world impact (no call site removes a session's listeners piecemeal without also closing/discarding the session itself) but a real gap in the public API's own contract, fixed alongside the `MediaRouter.terminate()`/`clearTransport()` leak audit below.
...
- **`sendSdpInfo(sdpInfo: SdpInfoEntry[])`** — the SDP-driven session factory, called once per
  ...
  and `VideoTagPlayer.setAudioInfo()`'s doc entries in `05-video-tag-player.md`) exist so the
...
- **`handleVideoData(session, playMode, streamData, videoInfo, isMetaImage?)`** — the video data
  ...
  in `05-video-tag-player.md`), wires timestamp/error/resize/framerate callbacks onto it,
...
- **`createAudioPlayer`/`deleteAudioPlayer`** — lifecycle for the standalone `AudioPlayerLike`. **2026-09-08**: `createAudioPlayer` now also pushes `this._audioEncoderMode` into the freshly-created player's new optional `AudioPlayerLike.audioEncoderMode` field, so the `audioencodermode` attribute selects the **decode** tier as well as the encode one. `set audioEncoderMode()` correspondingly gained a second line beside its existing `_videoPlayer?.setAudioEncoderMode?.(mode)` that re-pushes into an already-created `audioPlayer` — but note the deliberate asymmetry between the two: the video player's is a live swap, whereas `AudioPlayerGxx` only *reads* the field inside `audioInit()`, so a mid-session change there takes effect at the next `audioInit()` (next codec change or next session) rather than swapping a decoder out from under the running Web Audio scheduling path. See `06-listen-audio.md`'s `AACWebCodecsAudioDecoder` section for what the field actually selects between.
...
- **`terminate(func?)`** — full teardown: closes player, deletes audio player, terminates talk, signals `rtpClientCallback('close', '')` (which is what actually reaches `RtpClient.mediaRouterMessage('close', ...)` and cascades to closing every `RtpSession`/`RTCPSession`), resets NTP/profile state. **Fixed 2026-09-08**: also now unconditionally clears `minimapUpdateTimer` (`clearInterval`, resets `minimapTarget`/`minimapRefreshInterval`) and closes+nulls `backupProvider` (`closeStream()`, resets `isBackup`) if either is still active. Previously neither was touched here: the minimap `setInterval` created by `handleMinimapCommand({mode:'on', ...})` only ever got cleared by an explicit `{mode:'off'}` command, so a channel closed while minimap was on left that interval running forever, holding a live reference to the whole `MediaRouter` instance (and everything it in turn references) with no way for anything outside the class to reach and clear it — a genuine, unbounded memory leak, one per such session. `backupProvider` had the identical gap for an in-progress backup/export never explicitly `{command:'stop'}`-ed before teardown. See `02-network.md`'s matching `clearTransport()` History entry (same audit) and `MEMORY.md`.
...
is in `11-canvas-tag-player.md` and the `worker/videoDecoder` section of
