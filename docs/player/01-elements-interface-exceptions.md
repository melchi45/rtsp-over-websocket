...
**Version:** 1.7.7 · **Author:** Youngho Kim
...
| 2026-09-08 | Fixed a real memory leak, reported directly by the user as ~400MB used by a `<rtsp-over-websocket>` tag that was merely mounted in the DOM — `play()` had never even been called. Root cause: `connectedCallback()` unconditionally, eagerly called `loadFfmpegAACDecoder()` (see its own doc comment above `play()`'s Method Analysis entry) — downloading and asm.js-compiling `ffmpegAAC.decoder.js`, whose vendored build reserves a fixed 160MB `TOTAL_MEMORY` heap the instant it loads, needed only for canvas-tag-mode AAC audio (`AudioPlayerGxx`/`AACAudioDecoder`) — regardless of whether `play()` would ever be called, whether canvas mode would ever be selected (only decided later, from the first video frame, in `MediaRouter.selectVideoPlayer()`), or whether the stream carried AAC audio at all. Moved the call to the top of `play()` instead — still ahead of `AudioPlayerGxx.audioInit()`'s synchronous `AACAudioDecoder` construction on the first audio RTP frame (real SETUP/PLAY negotiation and buffering time in between), but now tied to an actual attempt to play rather than to the tag merely existing. `MediaRouter.createAudioPlayer()`'s own call site was considered and rejected as the new home for this instead: it runs in the exact same synchronous tick as `audioInit()` (`MediaRouter.ts`'s `handleAudioData()`), leaving no lead time at all. See `MEMORY.md` for the full investigation, including why a `tagMode`-gated trigger (skip entirely for `VideoTagPlayer`/MSE sessions, which never touch this decoder) wasn't feasible — `tagMode` itself isn't decided until after this call would need to fire. |
...
- `static get observedAttributes()` (`:314-349`) — the ~32 attributes
  ...
  `limitwidth`, `limitheight`, `android`, `debug`, `audioencodermode`).
...
- `play()` (`:4769-`) — normalizes `mode` to `'live'` if unset; for `INSTANTPLAYBACK` just
