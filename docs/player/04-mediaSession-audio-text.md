...
| 2026-09-04 | Follow-up, reported directly by the user (live-camera testing, "too many logs" from exactly `G711Session` and the entry above's `info`-level classification): reclassified `G711Session`/`G726Session`/`OPUSSession`'s single per-packet trace from `info` to `debug`. Reasoning at the time was "it's also the frame" — but that made these audio codecs' only trace line the same severity as a genuinely meaningful milestone (`H264Session`'s frame-complete, `AACSession`'s per-AU) despite firing far more often (audio packetization is typically ~50/s vs. ~15-30 video frames/s) and carrying no distinct "assembled" event to justify the promotion. `depacketize()`-level output is now uniformly `debug` regardless of whether a given codec happens to have a separate reassembly step — `AACSession`'s own `debug`(packet)/`info`(AU) split is unaffected, since AAC genuinely does have two distinct moments. Paired with `01-elements-interface-exceptions.md`'s matching default-level change (`'info'` → `'warning'`) — together, `debug["mediaSession"] = ["audioSession"]` with no explicit `level` now shows nothing, and `{"level":"info", ...}` shows only `AACSession`'s AU-complete lines, not a stream of G.711/G.726/Opus packet noise. |
