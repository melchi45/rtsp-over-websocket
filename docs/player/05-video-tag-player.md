...
**Version:** 1.2.22 · **Author:** Youngho Kim
...
| 2026-09-08 | Scope note added to "Audio encoder selection: WASM vs. WebCodecs": the `audioencodermode` attribute now *also* selects an AAC **decode** tier (new `AACWebCodecsAudioDecoder`, `06-listen-audio.md` v1.2.0). No code in this class changed — that half is canvas-tag-mode-only (`AudioPlayerGxx` is never constructed for a `VideoTagPlayer` session), so the two halves are mutually exclusive per stream; the note exists so a reader of this section doesn't assume the attribute is encode-only. |
| 2026-09-10 | Fixed a real bug reported directly by the user, root-caused together via `console.trace()` diagnostics added to `StreamPlayer.ts`/`RTSPOverWebSocket.ts`: clicking Mute -> Unmute -> Mute (Live mode) on an Opus-audio camera triggered a full, unexplained RTSP TEARDOWN+reconnect. `mute()`/`unmute()` themselves are local-only (confirmed via the diagnostics — no `open()`/`close()` ever appeared in their call stack); the reconnect always originated from `RTSPOverWebSocket.ts`'s error-driven `stop()`+`play()` retry (error code `0x030A`), thrown from this class's `appendSegmentToSourceBuffer()`'s `sourceBuffer.appendBuffer()` call. Root cause: `ControlVolume()`'s mute branch set `dummyAudio = true` in addition to `videoElement.muted = true` (the latter already fully silences output on its own) — `dummyAudio` means "no real audio data is available" (its other writers: the initial default before the first real sample, and `onWaitingPackets()` on genuine packet loss), not "the user muted it," and the camera keeps sending real RTP audio regardless of local mute state. `onAudioData()` resets `dummyAudio` back to `false` on the very next real sample, so the mute-forced `true` only ever won a brief race — but if a video segment got built in that window, `makeDummyAudio()` fabricates an **AAC**-coded dummy sample (hardcoded, see its own comment) and muxes it into the audio track regardless of the actual negotiated codec — this camera's was Opus (confirmed from its real DESCRIBE/SDP response, `a=rtpmap:110 opus/48000/2`) — a codec mismatch the browser's MSE parser rejects. Fixed by removing `dummyAudio = true` from `ControlVolume()`'s mute branch entirely; `videoElement.muted` alone is sufficient for the user-facing mute requirement. **Not yet fixed, same latent risk**: `makeDummyAudio()`'s hardcoded `'AAC'` codec is still reachable via its other two legitimate triggers (initial default, real packet loss) and would hit the identical codec-mismatch crash on any non-AAC-audio camera (Opus, G.711, G.726, ...) if a segment happens to build during one of those windows too — worth a follow-up fix (have `makeDummyAudio()` synthesize silence in whatever codec `this.audioCodecInfo`/`setAudioInfo()` actually established, not a fixed AAC frame). See `MEMORY.md` for the full live-debugging narrative (real device RTSP log first, then the `console.trace()` diagnostics that pinned it down). |
...
**Scope note (2026-09-08).** The same attribute now also selects an AAC **decode** tier — but
that half lives entirely in `AudioPlayerGxx`/`AACWebCodecsAudioDecoder`
(`06-listen-audio.md`), which is canvas-tag-mode-only and is never constructed for a
`VideoTagPlayer` session. So the two halves never both apply to one stream: in video-tag mode the
attribute means "which encoder transcodes G.711/G.726 to AAC" (this section); in canvas-tag mode
it means "which decoder turns AAC into PCM". Nothing in this class changed for that addition.
