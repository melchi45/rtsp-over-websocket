...
const LegacyCtor = loadLegacyModule<new () => LegacyG726Decoder>(
  'Listen/Decoder/audioDecoderG726_40.js',
  'G726_40_AudioDecoder',
  createAudioDecoderLegacySandbox()
);

const SAMPLE_BYTES = [0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x00, 0xff, 0x11, 0x22, 0x33];

describe('G726_40_AudioDecoder parity with the legacy player’s Listen/Decoder/audioDecoderG726_40.js', () => {
  it('decodes an identical Int16Array for the same input, evolving state the same way', () => {
    const legacy = new LegacyCtor();
    const ported = new G726_40_AudioDecoder();
