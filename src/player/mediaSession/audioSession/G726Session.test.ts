...
import { G726Session } from './G726Session';

interface LegacyG726Session {
...
const LegacyG726SessionCtor = loadLegacyModule<new () => LegacyG726Session>(
...
function rtpHeader(marker: boolean, timestamp: number): Uint8Array {
  ...
  header[0] = 0x80;
  header[1] = (marker ? 0x80 : 0x00) | 0x02;
...
describe('G726Session parity with the legacy player’s MediaSession/AudioSession/g726Session.js', () => {
  it('a single marker-bit packet produces an identical streamData/audioInfo callback', () => {
    const legacy = new LegacyG726SessionCtor();
    const ported = new G726Session();
    const info = { bitrate: 32000, clockFreq: 8000 };
    legacy.init(info);
    ported.init(info);
    ...
    legacy.depacketize(RTSP_INTERLEAVED, rtpHeader(true, 4000), payload);
    ported.depacketize(RTSP_INTERLEAVED, rtpHeader(true, 4000), payload);
