...
import { RTSPError } from './RTSPError';

const LegacyRTSPError = loadLegacyModule<LegacyErrorCtor>('Exception/RTSPError.js', 'RTSPError');

describe('RTSPError parity with the legacy player’s Exception/RTSPError.js', () => {
  it('matches on every field except name (deliberately rebranded — see RTSPError.ts)', () => {
    const options = { message: 'RTSP session teardown failed', channelId: 2, elementId: 'rtsp-over-websocket-player1', errorCode: 454, place: 'rtspClient.js' };
    const legacy = new LegacyRTSPError(options);
    const ported = new RTSPError(options);
    ...
    // after the base error class rather than 'RTSP Error' specifically —
    // this port intentionally does not reproduce that shared name
    // verbatim (see RTSPError.ts), so `legacy.name` is deliberately not
